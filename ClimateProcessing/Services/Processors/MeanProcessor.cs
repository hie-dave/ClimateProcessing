...
public class MeanProcessor : IVariableProcessor
{
    ...
    public async Task<IReadOnlyList<Job>> CreateJobsAsync(IClimateDataset dataset, IJobCreationContext context)
    {
        ...
        string jobName = $"calc_mean_{varInfo.Name}_{dataset.DatasetName}";
        ...
        // We have used the target variable's destination name in the
        // setattribute operator, so this needs to happen *after* the exprf
        // operator in which this variable is actually calculated. (As always,
