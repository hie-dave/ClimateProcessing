...
public class CordexDatasetTests : IDisposable
{
    ...
    public void GetVariableProcessors_ReturnsCorrectProcessors()
    {
        ...
        const int nfiles = 3;
        using TempDirectory directory = CreateTempFiles(dataset, ClimateVariable.MaxTemperature, nfiles);
        using TempDirectory directory2 = CreateTempFiles(dataset, ClimateVariable.MaxRelativeHumidity, nfiles);
        using TempDirectory directory3 = CreateTempFiles(dataset, ClimateVariable.MinTemperature, nfiles);
        using TempDirectory directory4 = CreateTempFiles(dataset, ClimateVariable.MinRelativeHumidity, nfiles);
    ...
    [Theory]
    [InlineData(CordexActivity.BiasCorrected)]
    [InlineData(CordexActivity.DD)]
    public void EnsureOutputFiles_UseRenamedVariableNames(CordexActivity activity)
    {
        IJobCreationContext context = new TestJobCreationContext();
        CordexDataset dataset = CreateDataset(activity);

        // Setup a test harness consisting of a few input files.
        const int nfiles = 16;
        CreateTempFiles(dataset, ClimateVariable.MinTemperature, nfiles);
        CreateTempFiles(dataset, ClimateVariable.MaxTemperature, nfiles);
        CreateTempFiles(dataset, ClimateVariable.MinRelativeHumidity, nfiles);
        CreateTempFiles(dataset, ClimateVariable.MaxRelativeHumidity, nfiles);
